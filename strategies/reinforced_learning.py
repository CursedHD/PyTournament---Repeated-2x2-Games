...
class reinforced_learning:
	...
	def move(self, game, player, history):
		...
		if history[-1][0] == "a":
			self.reflvl_a = self.reflvl_a + tournament.determine_payoff(game, "a", history[-1][1])[0]
	
		if history[-1][0] == "b":
			self.reflvl_b = self.reflvl_b + tournament.determine_payoff(game, "b", history[-1][1])[0]
